...
export function useElectronFolderLifecycle({
  ...
  const applySettingsFromMain = useCallback((settings) => {
    ...
    const {
      ...
      setTransferLayout: applyTransferLayout,
    ...
    if (settings.reviewModeEnabled !== undefined) {
      applyReviewModeEnabled?.(settings.reviewModeEnabled === true);
    }
    ...
    if (settings.transferLayout !== undefined) {
      applyTransferLayout?.(settings.transferLayout === "flat" ? "flat" : "structured");
    }
