...
const VideoCard = memo(
  ({
    ...
    const loadVideo = useCallback(async () => {
      if (hasAttemptedLoadRef.current || localLoading || videoRef.current)
        return;
    ...
    useEffect(() => {
      ...
      return () =>
        window.removeEventListener("resetVideoLoading", handleResetLoading);
    ...
    useEffect(() => {
      if (
        ...
        console.log(
          `🎬 VideoCard ${video.name}: Attempting to load (shouldLoad=${shouldLoad}, canLoadMore=${canLoadMore})`
        );
        loadVideo();
      }
    ...
    const getContent = () => {
      if (error) {
        ...
        return (
          <div
            className={`error-indicator error-${error.type}`}
            style={{
              display: "flex",
