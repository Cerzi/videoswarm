...
export const useLayoutManager = (videos, zoomLevel) => {
      ...
      if (window.electronAPI?.onSettingsLoaded) {
        window.electronAPI.onSettingsLoaded((settings) => {
          if (settings.layoutMode !== undefined) {
            console.log(
              "Loading layout mode from settings:",
              settings.layoutMode
            );
  ...
  const layoutMasonryVertical = useCallback(() => {
    ...
    videoItems.forEach((videoItem, index) => {
      ...
      const videoContainer = videoItem.querySelector(
  ...
  const layoutMasonryHorizontal = useCallback(() => {
    ...
    const totalHeight =
      allRows.length > 0 ? allRows.length * (fixedHeight + gap) : fixedHeight;
  ...
  const applyLayout = useCallback(
    (mode = layoutMode) => {
      ...
      console.log("Applying layout mode:", mode, "- Starting cleanup");

      // SET FLAG: Layout application in progress
      setIsApplyingLayout(true);

      // Preserve scroll position during layout changes
      const currentScrollY = window.scrollY;

      // PHASE 1 FIX: SAFER cleanup before layout switch

      // 1. Clear aspect ratio cache if it's getting large
      if (aspectRatioCacheRef.current.size > 500) {
        console.log(
          `🧹 Clearing aspect ratio cache (${aspectRatioCacheRef.current.size} entries)`
        );
        aspectRatioCacheRef.current.clear();
      }

      // 2. SAFER: Just pause videos, don't unload them completely
      const videoItems = grid.querySelectorAll(".video-item");
      let pausedCount = 0;
      ...
      videoItems.forEach((videoItem) => {
        ...
        if (video && !video.paused) {
          video.pause(); // Just pause, don't unload
          pausedCount++;
        }
      ...
      console.log(`⏸️ Paused ${pausedCount} videos for layout switch`);

      // 3. Clear cached measurements when switching layouts
      cachedGridMeasurementsRef.current = null;

      const clearLayoutFlag = () => {
        setTimeout(() => {
          setIsApplyingLayout(false);
          console.log(
            "Layout application complete - re-enabling visibility detection"
          );
        }, 500); // Give layout time to settle
      };

      // 4. Apply the layout (keep original timing)
      if (mode === "masonry-vertical") {
        setTimeout(() => {
          ...
          clearLayoutFlag(); // Clear flag after completion
        }, 50);
  ...
  const refreshMasonryLayout = useCallback(() => {
    ...
    if (
      ...
      console.log(
        "Skipping layout refresh - interaction or refresh in progress"
      );
      return;
    }
  ...
  return {
    ...
    setZoom,
    updateAspectRatio,
    isApplyingLayout, // ✅ Return the state value, not a function
  };
};
