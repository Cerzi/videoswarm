...
- `scripts/linux-fix.js`: electron-builder Linux wrapper that launches with `--no-sandbox --disable-setuid-sandbox`.
...
- Local file videos use `file://` URLs generated by `src/components/VideoCard/videoDom.js`; preserve Windows path handling and URL encoding behavior.
...
- Settings live in Electron user data via `electron-store`.
...
- Linux packages use the wrapper from `scripts/linux-fix.js` to disable Electron sandboxing for packaged launches.
